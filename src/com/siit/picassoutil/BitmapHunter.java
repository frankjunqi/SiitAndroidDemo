...
import static com.siit.picassoutil.Picasso.Priority.LOW;
import static com.siit.picassoutil.Utils.OWNER_HUNTER;
import static com.siit.picassoutil.Utils.VERB_DECODED;
import static com.siit.picassoutil.Utils.VERB_EXECUTING;
import static com.siit.picassoutil.Utils.VERB_JOINED;
import static com.siit.picassoutil.Utils.VERB_REMOVED;
import static com.siit.picassoutil.Utils.VERB_TRANSFORMED;
...
class BitmapHunter implements Runnable {
	...
	private static final RequestHandler ERRORING_HANDLER = new RequestHandler() {
		...
		public Result load(Request data) throws IOException {
			throw new IllegalStateException("Unrecognized type of request: "
					+ data);
		}
	};
	...
	BitmapHunter(Picasso picasso, Dispatcher dispatcher, Cache cache,
		...
		this.sequence = SEQUENCE_GENERATOR.incrementAndGet();// 自动增量的当前值。 每一个任务过来，就让sequence+1，保证线程安全
	...
	void attach(Action action) {
		...
		if (this.action == null) {
			...
			if (loggingEnabled) {
				if (actions == null || actions.isEmpty()) {
					log(OWNER_HUNTER, VERB_JOINED, request.logId(),
							"to empty hunter");
				} else {
					log(OWNER_HUNTER, VERB_JOINED, request.logId(),
		...
		// ordinal():返回的位置枚举常量的声明。第一个常数有一个顺序值为零。
	...
	void detach(Action action) {
		...
		if (picasso.loggingEnabled) {
			log(OWNER_HUNTER, VERB_REMOVED, action.request.logId(),
	...
	boolean cancel() {
		return action == null && (actions == null || actions.isEmpty())
	...
	static BitmapHunter forRequest(Picasso picasso, Dispatcher dispatcher,
		...
		for (int i = 0, count = requestHandlers.size(); i < count; i++) {
			...
			if (requestHandler.canHandleRequest(request)) {
				return new BitmapHunter(picasso, dispatcher, cache, stats,
						action, requestHandler);
			}
		}

		return new BitmapHunter(picasso, dispatcher, cache, stats, action,
				ERRORING_HANDLER);
	}
