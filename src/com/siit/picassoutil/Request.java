...
/** Immutable data about an image and the transformations that will be applied to it. 不可变数据对图像和应用的转换。
 * 	Request为图片请求之前的设置，包括Uri、默认图片、请求标识、图片转换Transformation动画，请求的优先级别、图片的旋转、目标图片的编码等；
 * 
 * */
public final class Request {
  ...
  /** Target image config for decoding. BitmapFactory.Options 主要是为了设置图片读取样式*/
  ...
  private Request(Uri uri, int resourceId, String stableKey, List<Transformation> transformations,
    ...
    if (transformations == null) {
      ...
      this.transformations = unmodifiableList(transformations);// 将参数中的List返回一个不可修改的List.
      														  //Transformation接口就是对图片进行转换之用
    }
    ...
    this.config = config;// 位图配置
    this.priority = priority;// 请求的优先级
  }
  ...
  String getName() {
    ...
    return Integer.toHexString(resourceId);//此方法返回的字符串表示的无符号整数参数所表示的值以十六进制（基数为16）.
  }

  /**
   * BitmapFactory.Options 主要是为了设置图片改如何读取
   * @return
   */
  ...
  /** Builder for creating {@link Request} instances. 创建一个Request实例*/
  public static final class Builder {
    ...
    /** Decode the image using the specified config. BitmapFactory.Options 主要是为了设置图片改如何读取*/
