...
/**
 * 这个Stats处理的是图片下载；主要看StatsHandler这个类中处理消息的主体,这个消息队列在this.
 * statsThread这个线程中进行消息轮询拿队列中的消息进行处理； performCacheHit() performCacheMiss()
 * performBitmapDecoded() performBitmapTransformed() performDownloadFinished()
 * 
 * 还有一些分发处理方法由外部调用，具体实现的还是上面的5中方法： dispatchBitmapDecoded()
...
class Stats {
	...
	private static class StatsHandler extends Handler {
		...
		// 这个消息队列在this.statsThread这个线程中进行消息轮询拿队列中的消息进行处理
