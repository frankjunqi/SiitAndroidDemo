...
import com.siit.picassoutil.RemoteViewsAction.AppWidgetAction;
import com.siit.picassoutil.RemoteViewsAction.NotificationAction;
import static com.siit.picassoutil.BitmapHunter.forRequest;
import static com.siit.picassoutil.Picasso.LoadedFrom.MEMORY;
import static com.siit.picassoutil.Picasso.Priority;
import static com.siit.picassoutil.PicassoDrawable.setBitmap;
import static com.siit.picassoutil.PicassoDrawable.setPlaceholder;
import static com.siit.picassoutil.Utils.OWNER_MAIN;
import static com.siit.picassoutil.Utils.VERB_CHANGED;
import static com.siit.picassoutil.Utils.VERB_COMPLETED;
import static com.siit.picassoutil.Utils.VERB_CREATED;
import static com.siit.picassoutil.Utils.checkMain;
import static com.siit.picassoutil.Utils.checkNotMain;
import static com.siit.picassoutil.Utils.createKey;
import static com.siit.picassoutil.Utils.isMain;
import static com.siit.picassoutil.Utils.log;
import static com.siit.picassoutil.Utils.sneakyRethrow;

/** Fluent API for building an image download request.通畅地API来构建一个映像下载请求。 */
...
public class RequestCreator {
	...
	private static int getRequestId() {
		...
		final CountDownLatch latch = new CountDownLatch(1);// CountDownLatch如其所写，是一个倒计数的锁存器，当计数减至0时触发特定的事件。利用这种特性，可以让主线程等待子线程的结束。
		final AtomicInteger id = new AtomicInteger();// AtomicInteger，一个提供原子操作的Integer的类。在Java语言中，++i和i++操作并不是线程安全的，在使用的时候，不可避免的会用到synchronized关键字。而AtomicInteger则通过一种线程安全的加减操作接口。
	...
	/** Internal use only. Used by {@link DeferredRequestCreator}.内部使用 */
	...
	/** Resize the image to the specified size in pixels.调整图片到指定的大小(以像素为单位)。 */
	...
	public Bitmap get() throws IOException {
		...
		String key = createKey(finalData, new StringBuilder());// 设置键

		Action action = new GetAction(picasso, finalData, skipMemoryCache, key,
				tag);
		return forRequest(picasso, picasso.dispatcher, picasso.cache,
				picasso.stats, action).hunt();
	}
	...
	public void fetch() {
		...
		if (data.hasImage()) {
			...
			Action action = new FetchAction(picasso, request, skipMemoryCache,
					key, tag);
	...
	public void into(Target target) {
		...
		if (!data.hasImage()) {
			...
			target.onPrepareLoad(setPlaceholder ? getPlaceholderDrawable()
					: null);
			return;
		}
		...
		Action action = new TargetAction(picasso, target, request,
				skipMemoryCache, errorResId, errorDrawable, requestKey, tag);
	...
	public void into(RemoteViews remoteViews, int viewId, int notificationId,
		...
		RemoteViewsAction action = new NotificationAction(picasso, request,
				remoteViews, viewId, notificationId, notification,
	...
	public void into(RemoteViews remoteViews, int viewId, int[] appWidgetIds) {
		...
		RemoteViewsAction action = new AppWidgetAction(picasso, request,
				remoteViews, viewId, appWidgetIds, skipMemoryCache, errorResId,
				key, tag);
	...
	private Drawable getPlaceholderDrawable() {
		if (placeholderResId != 0) {
			...
			return placeholderDrawable; // This may be null which is expected
