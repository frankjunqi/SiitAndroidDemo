...
import com.siit.picassoutil.Downloader.Response;

import static com.siit.picassoutil.Picasso.LoadedFrom.DISK;
import static com.siit.picassoutil.Picasso.LoadedFrom.NETWORK;
class NetworkRequestHandler extends RequestHandler {
  ...
  private Bitmap decodeStream(InputStream stream, Request data) throws IOException {
    ...
    boolean isWebPFile = Utils.isWebPFile(stream);// 判断此流是否是webP格式的图片流
    markStream.reset(mark);
    // When decode WebP network stream, BitmapFactory throw JNI Exception and make app crash.
    // Decode byte array instead(当遇到webP格式的图片文件，那么BitmapFactory将抛出JNI Exception 使程序crash，所以用byte数组进行代替)
    if (isWebPFile) {
      ...
      if (bitmap == null) {
        // Treat null as an IO exception, we will eventually retry.将空作为一个IO例外,我们最终将重试
