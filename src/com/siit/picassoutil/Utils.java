...
import static com.siit.picassoutil.Picasso.TAG;
	...
	*/
	...
	static String createKey(Request data, StringBuilder builder) {
		if (data.stableKey != null) {
			builder.ensureCapacity(data.stableKey.length() + KEY_PADDING);// 设置缓冲区的大小+每个键之间的留下50的大小
			builder.append(data.stableKey);
		}
		else if (data.uri != null) {
			String path = data.uri.toString();
			builder.ensureCapacity(path.length() + KEY_PADDING);// 设置缓冲区的大小+每个键之间的留下50的大小
			builder.append(path);
		}
		else {
			builder.ensureCapacity(KEY_PADDING);// 设置缓冲区的大小+每个键之间的留下50的大小
	 ...
	 * from the disk cache.如果头表示响应体加载从磁盘缓存
	 */
	...
	static long calculateDiskCacheSize(File dir) {
		...
		try {
			//StatFs 检索一个文件系统的整体信息空间。这是一个包装器为Unix statvfs()。
			StatFs statFs = new StatFs(dir.getAbsolutePath());
			//getBlockCount()文件系统块的总数。这对应于Unix { @code statvfs。}。getBlockSize()大小,以字节为单位,一块的文件系统。这对应于Unix { @code statvfs。 }。
			long available = ((long) statFs.getBlockCount()) * statFs.getBlockSize();
			// Target 2% of the total space.
			size = available / 50;
		} catch (IllegalArgumentException ignored) {}

		// Bound inside min/max size for disk cache.绑定在最小/最大磁盘高速缓存的大小。
	...
	private static class PicassoThread extends Thread {
		...
		@Override
		public void run() {
			//THREAD_PRIORITY_BACKGROUND :标准优先级后台线程。这给你的线程一个略低于正常优先级,这样它将有更少的机会影响用户界面的响应能力。
